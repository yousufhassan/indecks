<template>
  <li class="card-box" :class="card.color">
    <transition name="flip">
      <article
        @keypress.enter="flipCard(card, $event)"
        :key="card.flipped"
        class="single-card"
      >
        <button type="button" class="delete-icon" @click="deleteCard(card.id)">
          ...
          <h2 class="title">
            <h2 v-if="card.flipped"><div v-html="htmlReturningFn"></div></h2>
            <h2 v-else>{{card.front}}</h2>
          </h2>
          <label>
        <input
          v-model="answer"
...
export default {
  ...
  methods: {
    flipCard(card) {
      ...
      // If we are flipping to the back, compare answers.
      // if (card.fliiped) {
        // use this.answer to compare to actual answer ie "back"
      // }
    },
    deleteCard(id) {
    ...
    likeCard(card) {
      ...
      // When the card is liked send it to back end as another training prompt
